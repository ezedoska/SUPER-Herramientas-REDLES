...
def go(engine,ui,version,MainWindow):
    """
    ...
    browser.open('https://registroefectores.desarrollosocial.gov.ar/logueo.asp', verify=False)
    ...
    if browser.find(string='cerrar sesión usuario'):
        usr = pd.read_sql_query(f"""SELECT nombres 
                                    FROM operadores 
        ...
        return MainWindow.setWindowTitle(
            f"[ Herramientas REDLES 2020 - v.{version} ] - " +
            F"Bienvenide {usr['nombres'].iloc[0]}.{random.choice(redleshello)}"
        )
    else:
        return MainWindow.setWindowTitle(
            f"[ Herramientas REDLES 2020 - v.{version} ] - LOGIN INCORRECTO."
        )
